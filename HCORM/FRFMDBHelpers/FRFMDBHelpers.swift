...
class FRFMDBHelpers : NSObject {
    
    func copyDatabaseToDocumentFolder(databaseName: String, databaseExtention: String)->String {
        return self.copyDatabaseToDocumentFolder(databaseName, databaseExtension: databaseExtention, databaseFolder: "")
    }
    
    func copyDatabaseToDocumentFolder(databaseName: String, databaseExtension: String, databaseFolder: String)->String {
        // Initialize variable
        ...
        var databasePath = documentDirectory.stringByAppendingString(NSString(format:"/%@.%@", databaseName, databaseExtension))
